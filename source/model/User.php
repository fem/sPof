...
abstract class User extends AbstractModelWithId
{
    ...
    protected static function validate(array $input)
    {
        self::getValidator($input)
            ->isNoId('user_id', 'Ungültige user id.')
            ->isEmpty('name', 'Es wurde kein Name angegeben.')
            ->isEmpty('email', 'E-Mail Adresse darf nicht leer sein.')
            ->isNoEmail('email', 'Es wurde keine Email Adresse angegeben.')
            ->byRegex(
                'passphrase',
    ...
    public static function updatePassword($user_id, $password)
    {
        self::getValidator(['user_id' => $user_id, 'password' => $password])
            ->isNoId('user_id', 'Ungültige user id.')
            ->byRegex('password', InvalidParameterCheck::REGEX_ACCEPTABLE_PASSWORD, 'Passwort darf nicht leer sein.')
            ->validate();
    ...
    {
        self::getValidator($input)
            ->isEmpty('name', 'Username has to be not empty!')
            ->isEmpty('passphrase', 'Passphrase has to be not empty!')
            ->isEmpty('email', 'Email has to be not empty!')
            ->isEmpty('token', 'Token has to be not empty!')
            ->validate();
    ...
    public static function getProfileFieldNames()
    {
        return [
            ...
            'profile_telephone' => 'Festnetz',
            'profile_mobile' => 'Handy',
            'profile_messenger_icq' => 'ICQ',
            'profile_messenger_jabber' => 'Jabber',
